...
def show_students():
    ...
    try:
            ...
            if 'modal_id' in request.form:
                if request.form['modal_id'] == '':
                    if request.form['modal_fio'] != '':
                        user = User(request.form['modal_username'], request.form['modal_password'], request.form['modal_email'], false)
        ...
        flash("Ошибка!", 'error')
        students = Student.query.join(User, Student.user_id == User.id).filter(User.center_id == g.user.center_id).all()
...
def show_admins():
    ...
    try:
        if request.method == "POST":
            if 'modal_id' in request.form:
                if request.form['modal_id'] == '':
                    if request.form['modal_username'] != '':
                        user = User(request.form['modal_username'], request.form['modal_password'], '', 'true')
                        db.session.add(user)
                        db.session.commit()
                        flash("Успешно добавлено!")
                    else:
                        flash("Ошибка: Поле Логин пустое!", 'error')
                else:
                    if request.form['modal_username'] != '':
                        user = User.query.get(request.form['modal_id'])
                        user.username = request.form['modal_username']
                        user.password = request.form['modal_password']
                        db.session.commit()
                        flash("Успешно изменено!")
                    else:
                        flash("Ошибка: Поле ФИО пустое!", 'error')
                ...
                # users = User.query.filter(Teacher.fio.ilike('%'+request.form['fio']+'%'))\
    ...
    try:
        if request.method == "POST":
            if 'modal_id' in request.form:
                if request.form['modal_id'] == '':
                    if request.form['modal_fio'] != '':
                        user = User(request.form['modal_username'], request.form['modal_password'], '', false)
...
def show_centers():
    ...
    try:
    # if True:
        if g.user and g.user.is_authenticated and g.user.is_superuser and request.method == "POST":
            if 'modal_center_title' in request.form:
                if request.form['modal_center_id']:
                    center = Center.query.get(request.form['modal_center_id'])
                    center.title = request.form['modal_center_title']
                    db.session.commit()
                else:
                    center = Center(request.form['modal_center_title'])
...
def register():
    ...
    # if True:
        # if g.user and g.user.is_authenticated and g.user.is_superuser and request.method == "POST":
        #     if 'modal_admin_id' in request.form and request.form['modal_admin_id']:
        #         admin = Payment.query.get(request.form['modal_admin_id'])
        #         if admin:
        #             admin.username = request.form['username']
        #             admin.password = request.form['password']
        #             admin.email = request.form['email']
        #             admin.center_id = request.form['center_id']
        #             db.session.commit()
        #             flash("Оплата изменена!")
        #         else:
        #             flash("Ошибка!", "error")
        #     elif 'username' in request.form:
        #             user = User(request.form['username'], request.form['password'], request.form['email'], 'true')
        #             user.center_id = request.form['center_id']
        #             db.session.add(user)
        #             db.session.commit()
...
def register_old():
    ...
    if form.validate_on_submit():
        user = User(request.form['username'], request.form['password'], request.form['email'], true)
        # user.center_id = request.form['modal_center']
        db.session.add(user)
        db.session.commit()
        flash('Пользователь успешно создан!')
        return redirect(url_for('login'))
